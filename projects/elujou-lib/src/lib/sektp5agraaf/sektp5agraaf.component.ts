...
import { omavalitsussuhttjt } from '../graafSisend/omavalitsustjt';
...
@Component({
  selector: 'sektp5agraaf',
  templateUrl: './sektp5agraaf.component.html',
  styleUrls: [ './sektp5agraaf.component.scss' ],
  standalone: true,
...
export class Sektp5agraafComponent implements OnInit  {
  ...
  kysiTJT() {   //Ettevõtte tööjõu tootlikkus.'0
    for (let key in omavalitsussuhttjt) {
      if (omavalitsussuhttjt.hasOwnProperty(key)) {
        this.suhtarvArray2.push(omavalitsussuhttjt[key]);
      }
    }
    ///console.log("trykimeterviku", this.suhtarvArray2); //õige
    this.oigeSuhtarv= this.suhtarvArray2.filter(e=>{
      return e.vald === this.omavNo;////omavalitsuse number;
  })
         ...
         //   left: '1%'
          ...
          tooltip: {
            ...
            formatter: function(params) {
              ...
              if (params.value[1] == 2) {var zz = " Omakapitali puhasrentaablus"}
